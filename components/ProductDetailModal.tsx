...
const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ product, onClose }) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in overflow-hidden"
      onClick={onClose}
    >
      <div 
        className="bg-white w-full max-w-5xl max-h-[90vh] rounded-[2rem] shadow-2xl relative animate-scale-up flex flex-col overflow-hidden"
        onClick={handleContentClick}
      >
        {/* Close Button */}
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 bg-white/50 hover:bg-white rounded-full transition-colors text-gray-600 hover:text-red-500 shadow-sm"
        >
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="overflow-y-auto custom-scrollbar flex flex-col md:flex-row h-full">
            ...
            {/* Left Side: Visuals */}
            <div className="w-full md:w-3/5 bg-gray-100 p-6 h-fit min-h-full">
            ...
            <div className="w-full md:w-2/5 p-8 md:p-10 flex flex-col bg-white h-fit min-h-full">
